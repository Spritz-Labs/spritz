...
export function usePhoneVerification(userAddress: Address | null) {
  ...
  useEffect(() => {
    ...
    const fetchPhoneStatus = async () => {
      ...
      if (fetchError) {
        console.error("[usePhoneVerification] Error fetching status:", fetchError);
      } else if (data) {
  ...
  const lookupByPhone = useCallback(async (phone: string): Promise<PhoneData | null> => {
    ...
    try {
      ...
      if (lookupError) {
        console.error("[usePhoneVerification] Lookup error:", lookupError);
        return null;
      }
