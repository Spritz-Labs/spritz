...
export function usePolls(channelId: string | null, userAddress: string | null) {
    ...
    const fetchPolls = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(
                `/api/channels/${channelId}/polls?${params}`
            );
    ...
    const updatePoll = useCallback(
        async (
            ...
            if (!channelId || !userAddress) {
                throw new Error("Channel and user address are required");
            }
            const res = await fetch(
                `/api/channels/${channelId}/polls/${pollId}`,
                {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
        ...
        [channelId, userAddress, fetchPolls]
    );
    ...
    const deletePoll = useCallback(
        async (pollId: string) => {
            ...
            const res = await fetch(
                `/api/channels/${channelId}/polls/${pollId}?userAddress=${encodeURIComponent(
                    userAddress
                )}`,
                { method: "DELETE" }
            );
