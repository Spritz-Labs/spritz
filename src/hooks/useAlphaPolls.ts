...
import type { AlphaPoll } from "@/app/api/alpha/polls/route";

export function useAlphaPolls(userAddress: string | null) {
    const [polls, setPolls] = useState<AlphaPoll[]>([]);
    ...
    const fetchPolls = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/alpha/polls?${params}`);
    ...
    const createPoll = useCallback(
        async (
            ...
            const res = await fetch("/api/alpha/polls", {
                method: "POST",
    ...
    const vote = useCallback(
        async (pollId: string, optionIndex: number) => {
            ...
            const res = await fetch(`/api/alpha/polls/${pollId}/vote`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
