...
export function EncryptionIndicator({ isSecure, isLoading }: EncryptionIndicatorProps = {}) {
    ...
    // Legacy encryption (or not yet determined)
    if (isSecure === false) {
        return (
            <div className="flex items-center justify-center gap-1.5 py-2 text-xs text-amber-500/80">
                <svg
                    className="w-3.5 h-3.5"
                    fill="none"
                    viewBox="0 0 24 24"
                ...
                <span>Encrypted (peer hasn&apos;t upgraded yet)</span>
            </div>
        );
    }
    
    // Default state - encrypted but status unknown
    return (
        <div className="flex items-center justify-center gap-1.5 py-2 text-xs text-zinc-500">
            <svg
                className="w-3.5 h-3.5"
