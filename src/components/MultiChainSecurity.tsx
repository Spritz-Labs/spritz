...
function ChainStatusRow({ chain, onAddRecovery, isAddingRecovery, selectedChainForRecovery }: ChainStatusRowProps) {
    ...
    return (
        <div className={`bg-zinc-800/50 rounded-xl p-3 border transition-colors ${
            ...
            {/* Info for non-deployed chains */}
            {!chain.isDeployed && (
                <div className="mt-2 pt-2 border-t border-zinc-700/30 space-y-2">
                    <p className="text-[10px] text-zinc-500">
                        Safe will deploy with your first transaction on this chain
                    </p>
                    
                    {/* Show deployment cost estimate */}
                    {chain.deploymentEstimate && (
                        <div className={`rounded-lg p-2 ${
                            chain.deploymentEstimate.isSponsored 
                                ? "bg-emerald-500/10 border border-emerald-500/20" 
                                : "bg-amber-500/10 border border-amber-500/20"
                        }`}>
                            <div className="flex items-center justify-between">
                                <span className="text-[10px] text-zinc-400">Deployment Cost:</span>
                                {chain.deploymentEstimate.isSponsored ? (
                                    <span className="text-[11px] font-semibold text-emerald-400">FREE</span>
                                ) : (
                                    <span className="text-[11px] font-semibold text-amber-400">
                                        ~${chain.deploymentEstimate.estimatedCostUsd.toFixed(2)}
                                    </span>
                                )}
                            </div>
                            {!chain.deploymentEstimate.isSponsored && (
                                <p className="text-[9px] text-zinc-500 mt-1">
                                    {chain.deploymentEstimate.estimatedCostEth} {chain.symbol} @ {parseFloat(chain.deploymentEstimate.gasPriceGwei).toFixed(1)} gwei
                                </p>
                            )}
                        </div>
                    )}
