...
export function GoLiveModal({
    ...
    const startCamera = useCallback(
        async (videoDeviceId?: string, audioDeviceId?: string) => {
            try {
                ...
                if (!attached) {
                    console.warn(
                        "[GoLive] Video ref not available after retries, stream still captured"
                    );
                }
                ...
                if (err.name === "NotAllowedError") {
                    ...
                    message =
                        "Camera is in use by another app. Please close other apps using your camera.";
                } else if (err.name === "OverconstrainedError") {
                    message =
                        "Camera doesn't support the requested settings. Try selecting a different camera.";
                }
                setError(message);
    ...
    const content = (
        <>
            ...
            <div
                className={`relative overflow-hidden ${embed ? "flex-1 min-h-0 flex flex-col" : "flex-1"}`}
            >
                {ingestUrl && status !== "ending" ? (
                    ...
                    <div className={embed ? "flex flex-col flex-1 min-h-0" : "contents"}>
                        {/* Video preview area */}
                        <div
                            className={`relative bg-black ${embed ? "flex-1 min-h-0" : "w-full h-full"}`}
                        >
                            <video
                                ref={videoPreviewRef}
                                autoPlay
                                    ...
                                    <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/80 to-transparent pointer-events-auto">
                                        ...
                                        {hasBetaAccess && (
                                            ...
                                            >
                                                ...
                                                <div
                                                    className={`w-11 h-6 rounded-full transition-colors relative ${recordEnabled ? "bg-red-500" : "bg-zinc-600"}`}
                                                >
                                                    <div
                                                        className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition-transform ${recordEnabled ? "translate-x-5" : "translate-x-0.5"}`}
                                                    />
                                                </div>
                                            </button>
                                        )}
                            ...
                            {embed && (videoDevices.length > 1 || audioDevices.length > 1) && (
                                <div className="absolute top-2 right-2 flex items-center gap-1.5">
                                    ...
                                    <button
                                        ...
                                        className={`p-2 rounded-full transition-colors ${showDeviceSettings ? "bg-white/20" : "bg-black/60 hover:bg-black/80"}`}
                                    ...
                                    {videoDevices.length > 0 && (
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className="w-full px-2 py-1.5 bg-zinc-800 border border-white/10 rounded-lg text-white text-xs focus:outline-none focus:border-red-500 appearance-none"
                                            >
                        ...
                        {embed && (
                            <div className="shrink-0 p-3 bg-zinc-900 space-y-2.5 border-t border-zinc-800">
                                <input
                                    ...
                                    onChange={(e) => setTitle(e.target.value)}
                                    placeholder="Add a title..."
                                    className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-xl text-white text-sm placeholder-zinc-500 focus:outline-none focus:border-red-500"
                                />
                                ...
                                {hasBetaAccess && (
                                    <button
                                        type="button"
                                        onClick={() => setRecordEnabled(!recordEnabled)}
                                        className="w-full flex items-center justify-between px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-xl transition-colors"
                                    >
                                        ...
                                        <div
                                            className={`w-10 h-5.5 rounded-full transition-colors relative ${recordEnabled ? "bg-red-500" : "bg-zinc-600"}`}
                                        >
                                            <div
                                                className={`absolute top-0.5 w-4.5 h-4.5 rounded-full bg-white transition-transform ${recordEnabled ? "translate-x-[1.125rem]" : "translate-x-0.5"}`}
                                            />
                                        </div>
                                    </button>
                                )}
                                ...
                                <button
                                    onClick={handleGoLive}
                                    disabled={!cameraReady || isStarting}
                                    className="w-full py-3 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-400 hover:to-red-500 text-white font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2.5"
                                >
                                    {isStarting ? (
                                        <>
                                        ...
                                        <>
                                            <span className="w-3.5 h-3.5 bg-white rounded-full animate-pulse" />
                                            Go Live
                                        </>
                                    )}
                                </button>
    ...
    if (embed) {
        return <div className="flex-1 min-h-0 flex flex-col overflow-hidden">{content}</div>;
    }
