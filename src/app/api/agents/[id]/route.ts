...
export async function PATCH(
    ...
    try {
        ...
        const canEditOfficial =
            isAdmin && existingAgent.visibility === "official";
        ...
        if (visibility === "official" && !isAdmin) {
            return NextResponse.json(
                { error: "Only admins can set official visibility" },
                { status: 403 },
            );
        }
        ...
        if (webSearchEnabled !== undefined)
            updates.web_search_enabled = webSearchEnabled;
        if (useKnowledgeBase !== undefined)
            updates.use_knowledge_base = useKnowledgeBase;
        ...
        if (schedulingEnabled !== undefined)
            updates.scheduling_enabled = schedulingEnabled;
        if (eventsAccess !== undefined) updates.events_access = eventsAccess;
        if (publicAccessEnabled !== undefined)
            updates.public_access_enabled = publicAccessEnabled;
        ...
        if (suggestedQuestions !== undefined) {
            ...
            if (finalVisibility === "official") {
                const validatedQuestions = Array.isArray(suggestedQuestions)
                    ...
                    : [];
                updates.suggested_questions =
                    validatedQuestions.length > 0 ? validatedQuestions : null;
            }
        }

        // x402 configuration updates
        if (x402Enabled !== undefined) updates.x402_enabled = x402Enabled;
        if (x402PriceCents !== undefined)
            updates.x402_price_cents = Math.max(1, x402PriceCents);
        if (x402Network !== undefined) updates.x402_network = x402Network;
        if (x402WalletAddress !== undefined)
            updates.x402_wallet_address =
                x402WalletAddress?.toLowerCase() || null;
        if (x402PricingMode !== undefined)
            updates.x402_pricing_mode = x402PricingMode;
