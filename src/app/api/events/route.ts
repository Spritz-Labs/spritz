...
function deduplicateEvents(events: any[]): any[] {
    ...
    // Sort by created_at to keep the oldest when duplicates are found
    ...
    for (const event of sortedEvents) {
        ...
        if (!isDuplicate) {
            ...
            if (normalizedEventUrl && seenUrls.has(normalizedEventUrl.toLowerCase())) {
                isDuplicate = true;
            ...
            if (!isDuplicate && event.rsvp_url) {
                ...
                if (normalizedRsvpUrl && normalizedRsvpUrl !== normalizedEventUrl && seenUrls.has(normalizedRsvpUrl.toLowerCase())) {
                    isDuplicate = true;
                } else if (normalizedRsvpUrl && normalizedRsvpUrl !== normalizedEventUrl) {
        ...
        if (eventIds.length > 0) {
            ...
            if (interests) {
                for (const eventId of eventIds) {
                    const eventInterests = interests.filter(i => i.event_id === eventId);
                    interestCounts[eventId] = {
                        interested: eventInterests.filter(i => i.interest_type === 'interested').length,
                        going: eventInterests.filter(i => i.interest_type === 'going').length,
                    };
                }
            }
        }
